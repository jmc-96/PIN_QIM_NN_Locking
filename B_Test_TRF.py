...
def make_model_key(model_dir: str) -> str:
    p = Path(os.path.abspath(model_dir))
    # Ako je zadnji segment "WL", koristi roditeljski naziv + "_WL"
    # WL znaci Watermarked and Locked
    if p.name.lower() == "wl":
        return f"{p.parent.name}_WL"
    # Inače zadrži standardni basename
    return p.name

HEAD_HINTS = ("classifier", "score", "pre_classifier", "output.dense", "output_layer", "dropout", "pooler.dense")

# --------------------- Da li postoji klasifikaciona glava --------------------
...
def main():
    ...
    for mdir in args.model_dirs:
        ...
        try:
            ...
            results[name] = None
    print("--------------------------------------------------------------------")
    dodatak = input("Unesi dodatni detalj za naziv fajla: ").strip()
    base_dir = os.path.join(os.getcwd(), "test", "accuracy")
    os.makedirs(base_dir, exist_ok=True)
    filename = f"accuracy_results_{dodatak}.json" if dodatak else "accuracy_results.json"
